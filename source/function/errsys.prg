// Error handler system adapted to FiveLinux
...
#ifdef __HARBOUR__
   #define DLG_TITLE "FiveLinux for Harbour"
#else
   #define DLG_TITLE "FiveLinux for xHarbour"
#endif
...
static function ErrorDialog( oError )
   ...
   @ 9, 1.5 LISTBOX oLbx VAR n ITEMS aStack OF oDlg SIZE 418, 245

   @ 5, 31 BUTTON "See error.log file" SIZE 120, 20 ;
      ACTION WinExec( "gedit", "error.log" )

   if nButtons == 2
      @ 35, 40 BUTTON "_Quit" OF oDlg ACTION oDlg:End()
   else
      @ 35, 18 BUTTON "_Quit" OF oDlg ACTION oDlg:End()
   endif

   if oError:CanRetry
      @ 35, If( nButtons == 2, 22, 16 ) BUTTON "_Retry" ;
         OF oDlg ACTION ( lRet  := .t., oDlg:End() )
   endif

   if oError:CanDefault
      @ 35, 6 BUTTON "_Default"  OF oDlg ACTION ( lRet  := .f., oDlg:End() )
   endif

   ACTIVATE DIALOG oDlg CENTERED

   if lRet == nil
      CloseAllWindows( oDlg )
      ErrorLevel( 1 )
      QUIT
   endif
...
static function CloseAllWindows( oDlg )

   local n, aWnd := GetAllWin()

   for n = Len( aWnd ) to 1 step -1
      if aWnd[ n ]:ClassName() $ "TWINDOW,TDIALOG" .and. ;
         ! aWnd[ n ] == oDlg
         aWnd[ n ]:bValid = nil
         aWnd[ n ]:End()
      endif
