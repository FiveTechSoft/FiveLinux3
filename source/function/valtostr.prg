...
function FW_TToD( tDateTime )

   local dDate    := tDateTime

   if ValType( tDateTime ) == 'T'
#ifdef __XHARBOUR__
      dDate    := TTOD( tDateTime )
#else
      dDate    := STOD( Left( TTOS( tDateTime ), 8 ) )
#endif
   endif
...
function FW_DToT( dDate, cTime )    // cTime is optional

return STOT( DTOS( dDate ) + If( cTime == nil, "000000", StrTran( cTime, ':', '' ) ) )
